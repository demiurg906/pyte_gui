...
def run(exe):
    pyte_screen = pyte.Screen(*DEFAULT_SIZE)
    pyte_screen.set_mode(pyte.screens.mo.LNM)
    pyte_stream = pyte.Stream()
    pyte_stream.attach(pyte_screen)
    ...
    def main(loop, exe):
        ...
        if not isinstance(exe, list):
            raise ValueError('exe must be list, {} got instead'.format(type(exe)))
    ...
    def formated_display():
        res = ''
        for i, line in enumerate(pyte_screen.display):
            res += '{}: {}\n'.format(i, line)
        return res
    ...
    if sys.platform == 'win32':
        ...
        loop = asyncio.get_event_loop()
    try:
        loop.run_until_complete(main(loop, exe))
        return formated_display()
    finally:
        loop.close()
